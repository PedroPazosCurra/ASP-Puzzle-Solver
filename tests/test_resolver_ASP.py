...
def test_puzzle_inexistente():
    assert (resolver_ASP(modelo_sat, puzzle_invalido) == [1, "En resolver_ASP.py, se recibe un puzzle que no existe. Vigila que se pase bien.", None, None])

# Insatisfacibilidad si se pasa un modelo lógico UNSAT
def test_modelo_unsat():
    assert (resolver_ASP(modelo_unsat, puzzle_valido) == [1, "El programa que he inferido en base a tu mensaje no es resoluble. Asegúrate de escribir todas las variables del sistema, aunque no estén relacionadas con ningún elemento.", None, None])

# Error en grounding si se pasa modelo con mala sintaxis 
def test_modelo_invalido():
    assert( resolver_ASP(modelo_invalido, puzzle_valido) == [1, "Ha habido un problema en el proceso de grounding ASP", None, None])

# Solución en caso de modelo resoluble y puzzle correcto
def test_modelo_sat():
    status, model, has_array, imagenes_array = resolver_ASP(modelo_sat, puzzle_valido)
